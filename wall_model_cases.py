...
INPUT_TURB_FILES = dict(zip(TURB_CASES, [f'./data/{case}_data.pkl' for case in TURB_CASES]))
...
for angle in TBL_ANGLES:
    INPUT_TURB_FILES[f'TBL_{angle}'] = f'./data/TBL_{angle}_data.pkl'
...
for subcase in SUBCASES:
    INPUT_TURB_FILES[f'apg_{subcase}'] = f'./data/apg_{subcase}_data.pkl'
...
RE = [2, 10]
for re in RE:
    INPUT_TURB_FILES[f'aairfoil_{re}M'] = f'./data/aairfoil_{re}M_data.pkl'
...
NACA_SUB = ['10', '4', '2', '1']
for sub in NACA_SUB:
    INPUT_TURB_FILES[f'naca_4412_{sub}'] = f'./data/naca_4412_top{sub}n_data.pkl'
    TURB_CASES.append(f'naca_4412_{sub}')
    DATASET_PLOT_TITLE[f'naca_4412_{sub}'] = f'NACA4412 Re={sub}00000'
INPUT_TURB_FILES[f'naca_0012'] = f'./data/naca_0012_top4n12_data.pkl'
INPUT_TURB_FILES[f'naca_0025'] = f'./data/naca_0025_data.pkl'
...
for case in LBL_CASES:
    INPUT_TURB_FILES[f'FS_{case}'] = f'./data/falkner_skan_data_{case}.pkl'
...
for case in CURVE_CASES:
    INPUT_TURB_FILES[f'curve_{case}'] = f'./data/curve_{case}_data.pkl'
