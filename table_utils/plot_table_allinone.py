...
inputs_num = [0,1,2,3,4,5]
inputs_num = [0,1,2]
df_all = pd.DataFrame()

df = pd.read_csv(f'../testing_results/renamed/inputs1.csv')
df_all['Test Case'] = df['Test Case']
df_all['Log law'] = df['Log law']
df_all['abs_error'] = df['abs_error']

for inputs in inputs_num:
    df = pd.read_csv(f'../testing_results/renamed/inputs{inputs}.csv')
    df_all[f'inputs{inputs}'] = df['BFM']

df_all['Training?'] = [dataset in [DATASET_PLOT_TITLE[dataset] for dataset in training_datasets] for dataset in df['Test Case']]
...
styled = (
    ...
    .format({
             'Log law': lambda x: color_bars(x, 100), 
             'inputs0': lambda x: color_bars(x, 100), 
             'inputs1': lambda x: color_bars(x, 100), 
             'inputs2': lambda x: color_bars(x, 100), 
             'inputs3': lambda x: color_bars(x, 100), 
             'inputs4': lambda x: color_bars(x, 100), 
             'inputs5': lambda x: color_bars(x, 100), 
             'Training?': checkmark,
             'abs_error': checkmark})
    .set_table_styles([
        {
...
# note_html = '<p style="text-align: left; font-size: 30px;"><b>Note:</b> Colored line indicate worse performance compared to the log law.</p>'
# html_code = note_html + html_code


with open(f'./tables/all_model_012.html', 'w') as f:
    f.write(html_code)
